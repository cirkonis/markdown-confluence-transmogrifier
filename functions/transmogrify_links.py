...
def transmogrify_links(line):
    ...
    for link_text, md_link_path in link_results:
        # Generate the Confluence page title
        confluence_page_title = md_link_path.split('/')[-1].replace('.md', '').replace('-', ' ').replace('_', ' ').capitalize()

        # Construct the new Confluence URL
        confluence_url = f"{confluence_base_url}{confluence_page_title.replace(' ', '+')}"

        # Replace only the current link (specific match) instead of all at once
        line = line.replace(f'[{link_text}]({md_link_path})', f'[{link_text}]({confluence_url})')
